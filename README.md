# Veyloora — mindfulness coaching

Static marketing site (HTML / CSS / JS, no build step) hosted on GitHub Pages
at `veyloora.shop`.

> Generated from `C:\Users\souha\coaching-sites-factory` (content file `sites/veyloora.mjs`).
> To change the content, edit that file and run `node build.mjs veyloora` — editing the
...
| 🟠 Important | `contact@veyloora.shop` doesn't exist yet: set up free email forwarding in Namecheap (*Domain List → Manage → Redirect Email*). | Namecheap |
...
| 🟡 Later | Prices ($29 / $89 / $189) and plan contents should match what you actually sell. | `index.html` `#pricing` |
...
```
Coaching in mindfulness and meditation, delivered online: one-on-one video sessions and small live group practice covering meditation foundations, stress and overwhelm at work, sleep and wind-down routines, and breathwork. Clients follow an 8 to 12 week program with short daily practices and written check-ins from their coach. This is not therapy, medical care or mental health treatment. Services are sold as month-to-month subscriptions from $29 to $189 per month, cancellable at any time. No physical products are sold or shipped. Site: veyloora.shop
```
